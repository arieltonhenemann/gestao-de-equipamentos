...
export default function TermoModal({ func, isOpen, onClose }: TermoModalProps) {
    const celular = func.celulares && func.celulares.length > 0 ? func.celulares[0] : null;
    const chip = func.chips && func.chips.length > 0 ? func.chips[0] : null;
    const notebook = func.notebooks && func.notebooks.length > 0 ? func.notebooks[0] : null;

    const [imprimirCelular, setImprimirCelular] = useState(false);
    const [imprimirNotebook, setImprimirNotebook] = useState(false);
    const [imprimirChip, setImprimirChip] = useState(false);

    useEffect(() => {
        if (isOpen) {
            setImprimirCelular(!!celular);
            setImprimirNotebook(!!notebook);
            setImprimirChip(!!chip);
        }
    }, [isOpen, celular, notebook, chip]);

    if (!isOpen) return null;

    const temAlgum = celular || notebook || chip;

    const handlePrint = () => {
        let finalHtml = '';
        
        if (imprimirCelular && celular) {
            const content = document.getElementById(`termo-celular-${func.id}`);
            if (content) finalHtml += `<div class="term-page">${content.innerHTML}</div>`;
        }

        if (imprimirNotebook && notebook) {
            const content = document.getElementById(`termo-notebook-${func.id}`);
            if (content) {
                if (finalHtml) {
        ...
        if (imprimirChip && chip) {
            const content = document.getElementById(`termo-chip-${func.id}`);
            if (content) {
                if (finalHtml) {
                    finalHtml += '<div style="page-break-before: always;"></div>';
                }
        ...
        if (!finalHtml) {
            alert("Selecione pelo menos um termo para imprimir.");
            return;
        }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-in fade-in duration-200 print:static print:inset-auto print:bg-transparent print:p-0 print:block">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 print:max-w-none print:max-h-none print:shadow-none print:block print:overflow-visible relative">
                ...
                <div className="flex items-center justify-between p-4 border-b border-slate-100 print:hidden bg-slate-50 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-4 bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm">
                            {celular && (
                                <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-slate-700">
                                    <input 
                                        type="checkbox" 
                                        checked={imprimirCelular} 
                                        onChange={(e) => setImprimirCelular(e.target.checked)}
                                        className="rounded border-slate-300 text-purple-600 focus:ring-purple-600"
                                    />
                                    <Smartphone size={16} className="text-slate-400" /> Celular
                                </label>
                            )}
                            {notebook && (
                                <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-slate-700">
                                    <input 
                                        type="checkbox" 
                                        checked={imprimirNotebook} 
                                        onChange={(e) => setImprimirNotebook(e.target.checked)}
                                        className="rounded border-slate-300 text-purple-600 focus:ring-purple-600"
                                    />
                                    <Laptop size={16} className="text-slate-400" /> Notebook
                                </label>
                            )}
                            {chip && (
                                <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-slate-700">
                                    <input 
                                        type="checkbox" 
                                        checked={imprimirChip} 
                                        onChange={(e) => setImprimirChip(e.target.checked)}
                                        className="rounded border-slate-300 text-purple-600 focus:ring-purple-600"
                                    />
                                    <CreditCard size={16} className="text-slate-400" /> Chip
                                </label>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                onClick={handlePrint}
                                disabled={!imprimirCelular && !imprimirNotebook && !imprimirChip}
                ...
                {/* Área de Impressão (Visível no Modal, usada para extrair o HTML) */}
                <div className="p-8 overflow-y-auto flex-1 bg-slate-100 flex flex-col gap-8 print:bg-white print:p-0 print:gap-0">
                    {!temAlgum ? (
                        <div className="text-center py-10 text-slate-500 bg-white rounded-xl border border-slate-200">
                            Este funcionário não possui nenhum celular ou notebook vinculado para gerar o termo.
                        </div>
                    ) : (
                        <>
                            {/* PREVIEW DO CELULAR */}
                            {celular && imprimirCelular && (
                                <div id={`termo-celular-${func.id}`} className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 max-w-3xl mx-auto w-full text-slate-900 text-[15px] leading-relaxed font-sans print:shadow-none print:border-none print:p-0 print:max-w-none">
                                    ...
                                    <div className="mb-4 pl-4 font-semibold text-justify">
                                        <p>Modelo: {celular.modelo_marca || 'Não informado'} {celular.armazenamento ? `${celular.armazenamento}` : ''}{celular.memoria ? `, ${celular.memoria} RAM` : ''}{celular.tela ? `, Tela ${celular.tela}` : ''}{celular.processador ? `, Processador ${celular.processador}` : ''}</p>
                                        <p>Número: {chip ? chip.numero : 'Não informado'}</p>
                                        <p>Número de série: {celular.serial || 'Não informado'}</p>
                                    </div>
                                    ...
                                    <p className="mb-4 text-justify">
                                        A empresa fornece a cada funcionário um plano de dados de 15GB por mês para uso relacionado ao trabalho. Este plano de dados é considerado adequado para as necessidades de trabalho mensais.
                                    </p>
                                    
                                    <p className="mb-4 text-justify">
                                        Caso o funcionário esgote o plano de 15GB dentro do mês e seja necessário adicionar mais dados para continuar o trabalho, qualquer custo associado à adição de dados extras será cobrado do funcionário. A empresa irá notificar o funcionário antes de adicionar mais dados e o custo associado será deduzido do salário do funcionário no próximo ciclo de pagamento.
                                    </p>
                            ...
                            {/* PREVIEW DO NOTEBOOK */}
                            {notebook && imprimirNotebook && (
                                <div id={`termo-notebook-${func.id}`} className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 max-w-3xl mx-auto w-full text-slate-900 text-[15px] leading-relaxed font-sans print:shadow-none print:border-none print:p-0 print:max-w-none">
                                    <h1 className="text-center font-bold text-lg mb-8 uppercase underline underline-offset-4">
                                    ...
                                    <div className="mb-4 pl-4 font-semibold text-justify">
                                        <p>Modelo: {notebook.modelo_marca || 'Não informado'} {notebook.hd ? `SSD/HD ${notebook.hd}` : ''}{notebook.memoria ? `, ${notebook.memoria} de Ram` : ''}{notebook.processador ? ` e Processador ${notebook.processador}` : ''}</p>
                                        <p>Número de série: {notebook.serial || 'Não informado'}</p>
                                    </div>
                            ...
                            {/* PREVIEW DO CHIP */}
                            {chip && imprimirChip && (
                                <div id={`termo-chip-${func.id}`} className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 max-w-3xl mx-auto w-full text-slate-900 text-[15px] leading-relaxed font-sans print:shadow-none print:border-none print:p-0 print:max-w-none">
                                    <h1 className="text-center font-bold text-lg mb-8 uppercase underline underline-offset-4">
